...
document.onkeyup = function (data) {
    if (data.which == 27) { // Escape key
        $.post('http://whatnodepad/escape', JSON.stringify({}));
        if (notepadreader == true) {
            var x = document.getElementById("p1").value;
            $.post('http://whatnodepad/updating', JSON.stringify({ text: x}));
            $("#main").fadeOut();
...
function dropNotepad() {
    var x = document.getElementById("p1").value;
    if (x == ""){
        $.post('http://whatnodepad/droppingEmpty', JSON.stringify({}));
    }else {
        $.post('http://whatnodepad/dropping', JSON.stringify({ text: x}));
        $("#main").fadeOut();
        document.getElementById("p1").value = "";
        $("#main").css('display', 'none');
    }   
